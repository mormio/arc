...
def crop(grid: Grid, start: IntegerTuple, dims: IntegerTuple) -> Grid:
    ...
    return tuple(
        r[start[1] : start[1] + dims[1]]
        for r in grid[start[0] : start[0] + dims[0]]
    )
...
def hmatching(a: Patch, b: Patch) -> Boolean:
    ...
    return (
        len(set(i for i, j in toindices(a)) & set(i for i, j in toindices(b)))
        > 0
    )
...
def vmatching(a: Patch, b: Patch) -> Boolean:
    ...
    return (
        len(set(j for i, j in toindices(a)) & set(j for i, j in toindices(b)))
        > 0
    )
...
def asobject(grid: Grid) -> Object:
    ...
    return frozenset(
        (v, (i, j)) for i, r in enumerate(grid) for j, v in enumerate(r)
    )
...
def upscale(element: Element, factor: Integer) -> Element:
    ...
    if isinstance(element, tuple):
        ...
        for row in element:
            upscaled_row = tuple()
            for value in row:
                upscaled_row = upscaled_row + tuple(
                    value for num in range(factor)
                )
...
def replace(grid: Grid, replacee: Integer, replacer: Integer) -> Grid:
    """color substitution"""
    return tuple(
        tuple(replacer if v == replacee else v for v in r) for r in grid
    )
...
def switch(grid: Grid, a: Integer, b: Integer) -> Grid:
    """color switching"""
    return tuple(
        tuple(v if (v != a and v != b) else {a: b, b: a}[v] for v in r)
        for r in grid
    )
...
def center(patch: Patch) -> IntegerTuple:
    """center of the patch"""
    return (
        uppermost(patch) + height(patch) // 2,
        leftmost(patch) + width(patch) // 2,
    )
...
def canvas(value: Integer, dimensions: IntegerTuple) -> Grid:
    """grid construction"""
    return tuple(
        tuple(value for j in range(dimensions[1]))
...
def backdrop(patch: Patch) -> Indices:
    ...
    return frozenset(
        (i, j) for i in range(si, ei + 1) for j in range(sj, ej + 1)
    )
...
def inbox(patch: Patch) -> Indices:
    ...
    vlines = {(i, sj) for i in range(si, ei + 1)} | {
        (i, ej) for i in range(si, ei + 1)
    }
    hlines = {(si, j) for j in range(sj, ej + 1)} | {
...
def shoot(start: IntegerTuple, direction: IntegerTuple) -> Indices:
    ...
    return connect(
        start, (start[0] + 42 * direction[0], start[1] + 42 * direction[1])
    )
...
def frontiers(grid: Grid) -> Objects:
    ...
    column_indices = tuple(
        j for j, c in enumerate(dmirror(grid)) if len(set(c)) == 1
    )
