...
def apply_functions_with_timeout(
    n_functions_to_apply: int,
        ...
        for chosen_fn in chosen_fns:
            ...
            try:
                ...
                if len(sig.parameters) == 2:
                    ...
                    inputs_list = [
                        chosen_fn(input_grid, param)
                        for input_grid in inputs_list
                    ]
                else:
                    inputs_list = [
                        chosen_fn(input_grid) for input_grid in inputs_list
                    ]
                ...
                print(
                    f"Error applying function {chosen_fn.__name__}: {str(e)}"
                )
                continue
        ...
        try:
            ...
            if len(sig.parameters) == 2:
                param = random.randint(
                    1, 15
                )  # scale transform up to the size of m
...
while i_prob < synthetic_dataset_size:
    ...
    # apply them
    outputs, used_functions = apply_functions_with_timeout(
        n_functions_to_apply, inputs
    )
    problem_data = []
    ...
    if len(used_functions) > 0:
        for input, output in zip(inputs, outputs):
            problem_data.append({"input": input, "output": output})
